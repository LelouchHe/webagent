...
type ConfigLikeEvent = Extract<AgentEvent, { type: "session_created" | "config_option_update" }>;
...
function handleConnected(event: ConnectedEvent, sessions: SessionManager, config: EventHandlerConfig): void {
...
function handleConfigLikeEvent(event: ConfigLikeEvent, sessions: SessionManager, store: Store): void {
...
function handleMessageChunk(event: MessageChunkEvent, sessions: SessionManager): void {
...
function handleThoughtChunk(event: ThoughtChunkEvent, sessions: SessionManager): void {
...
function handleToolCall(event: ToolCallEvent, sessions: SessionManager, store: Store): void {
...
function handlePlan(event: PlanEvent, sessions: SessionManager, store: Store): void {
  sessions.flushBuffers(event.sessionId);
  sessions.state.patch(event.sessionId, { runtime: { streaming: { assistant: false, thinking: false } } });
...
function handlePromptDone(event: PromptDoneEvent, sessions: SessionManager, store: Store): void {
...
function dispatchAgentEvent(
  ...
  switch (event.type) {
    ...
    case "permission_request":
      return handlePermissionRequest(event, sessions, store, bridge, sseManager);
    case "prompt_done":
