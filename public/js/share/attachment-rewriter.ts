...
//
// Maps owner-side `/api/v1/tasks/<sid>/attachments/<file>` URLs (with
...
export function makeAttachmentRewriter(token: string): (src: string) => string {
  // Match `/api/v1/tasks/<sid>/attachments/<file>` with an optional query string.
  ...
  const re =
    /^\/api\/v1\/tasks\/[^/]+\/attachments\/([A-Za-z0-9._-]+)(?:\?.*)?$/;
