...
import {
  state, resetSessionUI, requestNewSession, sendCancel,
  getConfigOption, updateModeUI, updateStatusBar,
} from './state.ts';
  ...
  switch (cmd) {
    ...
    case '/switch': {
      ...
      try {
        ...
        const match = sessions.find((s: { id: string; title?: string | null }) =>
          s.id.startsWith(arg) ||
    ...
    case '/mode':
    case '/think': {
      const configMap: Record<string, string> = { '/model': 'model', '/mode': 'mode', '/think': 'reasoning_effort' };
