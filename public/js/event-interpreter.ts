...
export function classifyPermissionOption(
  ...
  const apiAction =
    k.includes("reject") || k.includes("deny")
      ? ("deny" as const)
      : ("resolve" as const);
 ...
 */
// eslint-disable-next-line complexity -- TODO: refactor to reduce branching
export function parseDiff(rawInput: RawInput | undefined): DiffLine[] | null {
  ...
  if (rawInput && typeof rawInput === "object") {
    const result: DiffLine[] = [];
    if (rawInput.path)
      result.push({ kind: "file", text: `*** ${rawInput.path}` });
