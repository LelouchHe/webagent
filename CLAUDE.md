...
- **`font-size: 0` to "hide" text is unreliable on iOS WebKit** — A common CSS trick to make a `::before` pseudo-element visually replace its host element's text is to set the host's `font-size: 0` so the original text collapses to zero width, then style the pseudo with its own font-size. This works on Chromium/Firefox but **leaks visible glyphs on iOS Safari** when the original text uses a fallback font (e.g. system symbol/emoji fallback for characters like `❯` U+276F). WebKit's font-fallback path doesn't strictly honor `font-size: 0` and renders the character at a "minimum readable" size. Symptom: spinner pseudo + faint original character side-by-side. **Fix: use `color: transparent` to hide the original text (every engine honors `color`), and `position: absolute` the pseudo on top so it doesn't stack inline.** First seen on the busy-state `#input-prompt` spinner — `❯` was leaking next to the Braille spinner glyph on iOS PWA.
