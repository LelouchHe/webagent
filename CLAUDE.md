...
- **iOS Safari / PWA keyboard** — iOS requires `.focus()` to originate from a synchronous user gesture (tap, click, keydown) for the virtual keyboard to appear. Calling `.focus()` from async callbacks (`setTimeout`, Promise `.then()`) puts the textarea into a "focused but no keyboard" state where subsequent taps also fail. The HTML `autofocus` attribute triggers this on page load. **Rule: never call `.focus()` outside a direct user gesture handler; never use `autofocus` on mobile-targeted inputs.** Real dogfood logs confirmed a worse platform stale-focus state too: `document.activeElement === textarea#input` while `visualViewport.height` stays unchanged and the keyboard remains closed; repeated blur+focus did not open the keyboard, but blurring by interacting with log text unlocked the next native tap. This is a WebKit/PWA limitation, not fully fixable in JS. Keep only a narrow stale-focus recovery in `public/js/input-focus-recovery.ts`: `pointerdown` may only mark a candidate; it must not immediately `blur()` because that cancels iPad Safari's native long-press Paste/Select menu. Only a short tap on the textarea itself may blur to unlock stale focus (`pointerdown`→`pointerup` ≤300ms and movement ≤10px); long press, movement, `pointercancel`, and touches on surrounding `#input-area` must not blur. iPad floating/split keyboards can leave `visualViewport.height === window.innerHeight` (`keyboardDelta = 0`) while the keyboard is usable, so viewport-not-shrunk is not enough evidence that it is safe to blur. Do not immediately refocus or add noisy permanent diagnostics.
