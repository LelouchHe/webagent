...
- **Web Push**: VAPID-based push notifications via `web-push`. Per-session visibility filtering: push is only suppressed for a session when a visible client is actively viewing that session. Subscriptions stored in SQLite. Notifiable events: `permission_request`, `prompt_done`, `bash_done`. Note: Web Push is browser-specific; other client types would need their own notification mechanism. The `/api/v1/push/*` namespace may warrant renaming (e.g. `/api/v1/web-push/*`) when the API stabilizes.
  - **Per-session visibility**: Each SSE client registers its push endpoint via `/api/v1/push/register-client` on connect and reports its current session via `POST /api/v1/clients/:clientId/visibility` with `{ visible: boolean, sessionId?: string }`. `sendToAll()` checks per-endpoint whether any mapped client is both visible **and viewing the notification's session** — only matching endpoints are skipped. This means a PWA viewing session A still receives push for session B completing in the background.
