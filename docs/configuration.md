...
| `title.model`                        | `["haiku", "flash-lite", "nano", "mini", "flash", "lite"]` | Array of case-insensitive substring patterns matched against the agent's reported `availableModels`. First match → used as title-session model; no match → inherit the agent's `currentModelId`. Default list targets the cheap/fast tier across major providers (Anthropic Haiku, OpenAI mini/nano, Gemini Flash, Cohere Lite). Set to `[]` to disable matching entirely; use a single-element array (e.g. `["claude-haiku-4.5"]`) to pin one specific model.                                |
