...
| `title.model`                        | `["haiku", "flash-lite", "nano", "mini", "flash", "lite"]` | String or array of case-insensitive substring patterns matched against the agent's reported `availableModels`. First match → used as title-session model; no match → inherit the agent's `currentModelId`. Default list targets the cheap/fast tier across major providers (Anthropic Haiku, OpenAI mini/nano, Gemini Flash, Cohere Lite). Set to `""` or `[]` to disable matching entirely; set to a single exact id to pin one specific model.                                |
