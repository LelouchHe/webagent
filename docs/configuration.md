...
- In practice the agent uses absolute paths most of the time, so this is rarely visible. If you want `pwd` and shell-relative paths to also match the task cwd, launch `webagent` from your project directory (`cd ~/myproject && webagent`).

**This does NOT affect WebAgent's own `!command` shell** — that always honors the current task's cwd (`spawn(..., { cwd: task.cwd })` in `routes.ts`), regardless of which agent is in use. The caveat above is purely about the shell tool that the Codex *agent itself* invokes.

This is by design in `codex-rs` (Zed's editor spawns one `codex-acp` per project, so process cwd ≡ task cwd by construction). Copilot CLI and Claude Code instead honor ACP `task.cwd` in their own shell tools too.
...
| `host`                               | `127.0.0.1`                 | Network interface to bind. Default `127.0.0.1` = loopback only (not reachable from LAN; pair with a reverse proxy / tunnel like cloudflared for external access). Set to `0.0.0.0` to listen on all IPv4 interfaces, `::` for IPv6/dual-stack, or a specific NIC IP on a multi-homed host. Prefer explicit `127.0.0.1` over `localhost` — the latter resolves via DNS and may pick IPv6 (`::1`) depending on `/etc/hosts` order. |
| `data_dir`                           | `data`                      | SQLite + uploads directory                                                                                                                                                                                                                                                                                           |
| `default_cwd`                        | `process.cwd()`             | Working directory for new tasks                                                                                                                                                                                                                                                                                   |
...
| `limits.cancel_timeout`              | `10000` (10s)               | Time to wait for an agent cancel acknowledgement before marking it unconfirmed; the task stays busy and cancel remains retryable. `0` disables the timeout marker.                                                                                                                                                |
...
| `push.global_visibility_suppression` | `true`                      | When `true`, a single client viewing task X suppresses push for task X on **all** endpoints/devices. Set to `false` to disable cross-device suppression as an emergency rollback without code change. See [Visibility Sync & Push Suppression](client-architecture.md#visibility-sync--push-suppression).      |
| `title.models`                       | `["haiku", "flash-lite", "nano", "mini", "flash", "lite"]` | Array of case-insensitive substring patterns matched against the agent's reported `availableModels`. First match → used as title-task model; no match → inherit the agent's `currentModelId`. Default list targets the cheap/fast tier across major providers (Anthropic Haiku, OpenAI mini/nano, Gemini Flash, Cohere Lite). Set to `[]` to disable matching entirely; use a single-element array (e.g. `["claude-haiku-4.5"]`) to pin one specific model.                                |
| `messages.unprocessed_ttl_days`      | `30`                        | Days before an unprocessed unbound inbox message is auto-cleaned. `0` = keep forever. Bound messages attached to tasks are not affected. See [Messages / Inbox](messages.md).                                                                                                                                     |
| `debug.level`                        | `"off"`                     | Inline log level — one of `off \| debug \| info \| warn \| error`. When `level != "off"`, frontend `log.*` records above that level emit both to the DevTools console and inline into the conversation flow (as system messages). Override per page via `?debug=<level>` URL param or persist a local browser override via `/log <level>` (`/log reset` follows config again). |
| `share.enabled`                      | `false`                     | Master kill switch for the share-link feature. When `false`, all `/api/v1/tasks/*/share*` and `/s/*` routes return 410 and `/share` slash commands are hidden. See [Share Links](share.md).                                                                                                                       |
...
The agent (Copilot CLI) runs its bash tool in a bash task that does **not** source user shell config files (`~/.bashrc`, `~/.zprofile`, `~/.zshrc`, etc.). Instead, the agent's bash tasks inherit the environment of the **parent process** — i.e. the WebAgent server.
