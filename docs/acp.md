...
| `mcpServers`                       | `webagent` (Task Server) | Client injects the `webagent` Task control-plane server (Streamable HTTP at `/mcp`, per-task capability Bearer token). The current S2 surface exposes `task_list`, `task_query`, `task_get_record`, `task_cancel`, `task_send`, and `task_update`; proposal tooling remains deferred |
