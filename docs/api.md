...
| Timestamps       | Two representations. Agent-facing and presentation fields (`createdAt`, the task snapshot, the share bundle `created_at`) render ISO-8601 UTC with `Z`. Store-shaped fields (`created_at`, `last_active_at`, `last_used_at`, `shared_at`) carry integer unix milliseconds. See [Timestamps](#timestamps). |
...
- **Store-shaped REST** — `GET /api/v1/tasks` (`created_at`, `last_active_at`),
  ...
  (`last_used_at`), `GET /api/v1/shares` (`created_at`), and every `shared_at`
  field carry the **stored integer unix milliseconds** unchanged
  (for example `1777410000000`), matching the `number` types in `src/types.ts`.
