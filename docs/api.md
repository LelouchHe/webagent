...
The reserved Root Task (`id = "root"`) cannot be deleted.

**Response** `204` (no body)

**Side effects:** Broadcasts `task_deleted` to all SSE clients, once per
deleted task. Each affected task's ACP execution is retired explicitly
(`task/delete` or `task/close` when the agent advertises support).
