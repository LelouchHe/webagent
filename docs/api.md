...
| Timestamps       | Two representations. Agent-facing and presentation fields (`createdAt`, the task snapshot, the share bundle `created_at`) render ISO-8601 UTC with `Z`. A `shared_at` that shares a JSON object with a rendered-ISO `created_at` also renders ISO; otherwise store-shaped fields carry integer unix milliseconds. See [Timestamps](#timestamps). |
...
Stored timestamps are unix milliseconds (`INTEGER`). How a response exposes
...
- **Same object, same shape** — a `shared_at` that lives in a JSON object next
  ...
  (`/share/preview`, where it is `null`).
- **Store-shaped REST** — `GET /api/v1/tasks` (`created_at`, `last_active_at`),
  `GET /api/v1/tasks/:id/events` (`created_at`), `GET /api/v1/recent-paths`
  (`last_used_at`), `GET /api/v1/shares` (`created_at`, `shared_at`), and every
  `shared_at` without a rendered-`created_at` sibling (`POST /share`,
...
`created_at` and `last_active_at` are the stored integer unix milliseconds
(see [Timestamps](#timestamps)); the task detail's `createdAt` is the
ISO-8601 rendering.
