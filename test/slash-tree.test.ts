  ...
  return {
    ...
    children: [
      ...
      },
      {
        ...
        desc: "Manage inbox",
        fetch: async () => [],
        toSpec: (item: any) => ({ primary: item.id, path: item.cwd }),
        children: [
          {
            name: "dismiss",
            desc: "Dismiss only",
      ...
      },
      {
        ...
        desc: "Manage tokens",
        fetch: async () => [],
        toSpec: (item: any) => ({ primary: item.name }),
        freeform: (q) =>
          /^[A-Za-z0-9_-]{1,64}$/.test(q.trim())
            ? {
                primary: `create token '${q.trim()}'`,
                onSelect: async () => {},
              }
            : null,
      },
...
describe("slash-tree — buildCandidates", () => {
  ...
  it("data items with current=true get prefix '*'", () => {
    const node: CmdNode = {
      name: "/model",
      fetch: async () => [],
      toSpec: (item: any) => ({ primary: item.name, current: item.cur }),
    };
  ...
  it("freeform appended after subcommands, before data, with prefix '›'", () => {
    const token: CmdNode = {
      ...
      toSpec: (item: any) => ({ primary: item.name }),
      freeform: (q) =>
        q ? { primary: `create '${q}'`, onSelect: async () => {} } : null,
    };
  ...
  it("collision: freeform suppressed when query matches an existing data primary", () => {
    ...
    const kinds = c.map((x) => x.kind);
    assert.equal(
      kinds.includes("freeform"),
      false,
      "freeform suppressed by collision",
    );
