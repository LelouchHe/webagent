...
assert.strictEqual(
  typeof document,
  "undefined",
  "Tests must run without DOM globals (no happy-dom)",
);
...
describe("interpretToolCall", () => {
  ...
  it("extracts command from rawInput with $ prefix", () => {
    const result = interpretToolCall("execute", "Run command", {
      command: "ls -la",
    });
  ...
  it("extracts path from rawInput when no command", () => {
    const result = interpretToolCall("read", "Read file", {
      path: "src/foo.ts",
    });
  ...
  it("prefers command over path", () => {
    const result = interpretToolCall("execute", "Run", {
      command: "npm test",
      path: "/tmp",
    });
