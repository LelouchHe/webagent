...
// `{ path, url }` where `url` is the signed form `…/images/<file>?sig=…&exp=…`.
...
//   1. signed URL → rewritten to `/s/<token>/images/<file>` (query stripped)
//   2. unsigned URL → rewritten the same way
//   3. unrelated `src` (data:, http://, relative) → returned unchanged

import { test } from "node:test";
import assert from "node:assert/strict";
import { makeImageRewriter } from "../public/js/share/image-rewriter.ts";

test("rewrites signed /api/v1/sessions image URL to viewer proxy", () => {
  const rw = makeImageRewriter("abcdefghijklmnopqrstuvwx");
  const signed =
    "/api/v1/sessions/sess123/images/1700000000.png?exp=1700003600&sig=deadbeef";
  assert.equal(rw(signed), "/s/abcdefghijklmnopqrstuvwx/images/1700000000.png");
});

test("rewrites unsigned /api/v1/sessions image URL", () => {
  const rw = makeImageRewriter("abcdefghijklmnopqrstuvwx");
  assert.equal(
    rw("/api/v1/sessions/sess123/images/foo.jpg"),
    "/s/abcdefghijklmnopqrstuvwx/images/foo.jpg",
  );
});
...
test("rewrites with sig=…&exp=… ordering as well", () => {
  const rw = makeImageRewriter("abcdefghijklmnopqrstuvwx");
  assert.equal(
    rw("/api/v1/sessions/s/images/x.webp?sig=abc&exp=999"),
    "/s/abcdefghijklmnopqrstuvwx/images/x.webp",
  );
});
...
test("token is URL-encoded in the rewritten path", () => {
  ...
  const rw = makeImageRewriter("a/b");
  assert.equal(rw("/api/v1/sessions/s/images/x.png"), "/s/a%2Fb/images/x.png");
});
