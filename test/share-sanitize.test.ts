...
describe("sanitize — Layer 1a structured rewrite", () => {
  it("rewrites cwd to <cwd>", () => {
    const out = sanitizeEventsForShare({
      events: [
        ev(1, "assistant_message", {
          text: `see ${CWD}/src/main.ts for details`,
        }),
      ],
      cwd: CWD,
      homeDir: HOME,
    ...
    assert.equal(out.events[0].data.text, "see <cwd>/src/main.ts for details");
  });

  it("rewrites homedir to <home> when not shadowed by cwd", () => {
    const out = sanitizeEventsForShare({
      events: [
        ev(1, "assistant_message", { text: `ls /Users/alice/other/path` }),
      ],
      cwd: CWD,
      homeDir: HOME,
      internalHosts: [],
    });
    assert.equal(out.events[0].data.text, "ls <home>/other/path");
  });
  ...
  it("cwd rewrite wins over homedir when cwd is subpath", () => {
    ...
    assert.equal(out.events[0].data.text, "cd <cwd>/subdir");
  });
  ...
  it("rewrites internal_hosts list", () => {
    const out = sanitizeEventsForShare({
      events: [
        ev(1, "assistant_message", {
          text: `curl https://internal.corp.com/api`,
        }),
      ],
      cwd: CWD,
      homeDir: HOME,
      internalHosts: ["internal.corp.com"],
    });
    assert.equal(out.events[0].data.text, "curl https://<internal-host>/api");
  });
  ...
  it("handles arrays (e.g. tool_call_update.content)", () => {
    const out = sanitizeEventsForShare({
      events: [
        ev(1, "tool_call_update", {
          status: "completed",
          content: [
            { type: "text", content: { text: `output: ${CWD}/log.txt` } },
          ],
        }),
      ],
      cwd: CWD,
  ...
  it("rejects github_pat tokens", () => {
    ...
    try {
      sanitizeEventsForShare({
        ...ctx,
        events: [
          ev(7, "assistant_message", {
            text: "token is github_pat_NOTAREAL00000000000000000000000",
          }),
        ],
      });
    } catch (e) {
    ...
    assert.throws(
      () =>
        sanitizeEventsForShare({
          ...
          events: [
            ev(1, "assistant_message", {
              text: "ghp_NOTAREAL0000000000000000",
            }),
          ],
        }),
    ...
    assert.throws(
      () =>
        sanitizeEventsForShare({
          ...
          events: [
            ev(1, "assistant_message", {
              text: `aws_secret_access_key=ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890+/=`,
            }),
          ],
        }),
      /AWS secret/,
...
describe("sanitize — API", () => {
  ...
  it("accepts StoredEvent shape (data as JSON string)", () => {
    ...
    assert.equal(out.events[0].data.text, "home: <home>");
  });
