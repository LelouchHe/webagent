...
describe("assertOwner", () => {
  describe("accepts", () => {
    ...
    it("Sec-Fetch-Site: same-origin", () => {
      assert.doesNotThrow(() => {
        assertOwner(
          fakeReq({ "sec-fetch-site": "same-origin", host: "localhost:6800" }),
        );
      });
    });
    ...
    it("Sec-Fetch-Site: none (address bar navigation)", () => {
      assert.doesNotThrow(() => {
        assertOwner(
          fakeReq({ "sec-fetch-site": "none", host: "localhost:6800" }),
        );
      });
    });
  ...
  describe("rejects", () => {
    it("naked curl (no Origin, no Sec-Fetch, no CF) — 'no_origin_no_sec_fetch'", () => {
      assert.throws(
        ...
        (e: unknown) =>
          e instanceof OwnerAuthError && e.reason === "no_origin_no_sec_fetch",
      );
    });
    ...
    it("Origin set but no Sec-Fetch-Site — 'origin_without_sec_fetch' (curl signature)", () => {
      ...
      for (const headers of samples) {
        assert.throws(
          ...
          (e: unknown) =>
            e instanceof OwnerAuthError &&
            e.reason === "origin_without_sec_fetch",
    ...
    it("Sec-Fetch-Site: cross-site — 'sec_fetch_cross_site'", () => {
      assert.throws(
        ...
        (e: unknown) =>
          e instanceof OwnerAuthError && e.reason === "sec_fetch_cross_site",
      );
    });
    ...
    it("Sec-Fetch-Site: same-site (sibling subdomain) — 'sec_fetch_cross_site'", () => {
      assert.throws(
        () => {
          assertOwner(
            fakeReq({ "sec-fetch-site": "same-site", host: "a.example.com" }),
          );
        },
        (e: unknown) =>
          e instanceof OwnerAuthError && e.reason === "sec_fetch_cross_site",
    ...
    it("CF Access header present but not an email shape — falls through to other checks", () => {
      ...
      assert.throws(() => {
        assertOwner(fakeReq({ "cf-access-authenticated-user-email": "" }));
      });
      assert.throws(() => {
        assertOwner(
          fakeReq({ "cf-access-authenticated-user-email": "not-an-email" }),
        );
      });
    });
  });
