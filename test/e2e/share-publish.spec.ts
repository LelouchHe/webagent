...
test("share: create preview, publish, public viewer renders without CSP violations", async ({ page, request }) => {
  ...
  await expect(page.locator("#messages")).toContainText("hello", { timeout: 10_000 });

  // /share → owner-side preview
  await sendPrompt(page, "/share");
  await expect(page.locator("#messages")).toContainText("share preview", { timeout: 5_000 });
  ...
  // /share publish → activate
  await sendPrompt(page, "/share publish");
  await expect(page.locator("#messages")).toContainText("share published", { timeout: 5_000 });
  ...
  await expect(viewer.locator("#messages .msg")).toHaveCount(await (async () => {
    // Wait for the fetch to have populated at least one row.
    await viewer.waitForFunction(() => document.querySelectorAll("#messages .msg").length > 0, null, { timeout: 10_000 });
    return await viewer.locator("#messages .msg").count();
  })());
  expect(viewerErrors, `unexpected CSP violations on viewer page: ${viewerErrors.join("\n")}`).toHaveLength(0);
  ...
  await viewer.close();
  expect(cspViolations, `unexpected CSP violations on owner page: ${cspViolations.join("\n")}`).toHaveLength(0);
});
