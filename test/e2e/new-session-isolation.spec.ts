...
test("creating a new session in one tab does not switch another tab away", async ({ browser }) => {
  ...
  await expect(pageB.locator(".msg.assistant").last()).toContainText("Echo: still on the original session");
});
