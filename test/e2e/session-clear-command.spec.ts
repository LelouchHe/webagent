...
test("/clear deletes current session and opens a fresh one in the same cwd", async ({ page }) => {
  await gotoConnected(page);
  const oldSessionId = await createNewSession(page);
  await sendPrompt(page, "stale content to be wiped");
  await expect(page.locator("#messages")).toContainText("stale content to be wiped");
  ...
  await expect(page.locator("#messages")).not.toContainText("stale content to be wiped");

  // Deleted session should not appear in switch menu
  await page.locator("#input").fill("/switch ");
  await expect(page.locator("#slash-menu.active")).not.toContainText(oldSessionId.slice(0, 8));
});
